#Execute: ~/Desktop/run_CPAchecker.sh
...
while IFS= read -r path; do
    ...
    start_time=$(gdate +%s%N)
    {
        docker run --rm --entrypoint /bin/bash -v "$root_directory":/liveness_analysis sosylab/cpachecker:4.0 -c "
            ...
            timeout "$timeout" /cpachecker/scripts/cpa.sh --config /cpachecker/config/terminationAnalysis.properties --preprocess --heap 10000M --64 --stats Benchmarks/\"$path_postfix\""
