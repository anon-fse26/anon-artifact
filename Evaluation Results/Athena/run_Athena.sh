...
athena_exe="$root_directory/Athena/cmake-build-debug/Athena"
benchmarks_directory="$root_directory/Benchmarks"

fse_original="$benchmarks_directory/FSE"
fse_BV="$benchmarks_directory/FSE_Bitvector_Mode"
fse_MA="$benchmarks_directory/FSE_Modulo_Arithmetic_Mode"
fse_MI="$benchmarks_directory/FSE_Mathematical_Integer_Mode"
...
find "$benchmarks_directory" -type f \( -name "*.c" -o -name "*.cpp" \) | sort | while read -r source_code; do
...
while IFS= read -r path; do
    ...
    output_file_path="$root_directory/Benchmarks/$directory_path/Output.txt"
    ...
    {
        docker run --rm -v "$root_directory":/liveness_analysis ${dockerName} /bin/bash -c " \
            cd /root/coar && \
            timeout ${timeout} ./main.exe -c ${cFlag} -p ${pFlag} /liveness_analysis/Benchmarks/\"$t2_file_path\""
