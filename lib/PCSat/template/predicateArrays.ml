...
module Make (Cfg : Config.ConfigType) (Arg : ArgType) : Function.Type = struct
  ...
  let gen_template ~tag ~ucore (hspace : HypSpace.hspace) =
    ignore tag;
    let template =
      match ucore with
    ...
    ( (DisjQPA, tmpl),
      (if Formula.is_true template.coeffs_bounds then []
       else [ (false, Coeff, Logic.ExtTerm.of_old_formula template.coeffs_bounds) ])
      @ (if Formula.is_true template.consts_bounds then []
         else [ (false, Const, Logic.ExtTerm.of_old_formula template.consts_bounds) ])
      @ (if Formula.is_true template.quals_bounds then []
         else [ (true(*ToDo*), Qual, Logic.ExtTerm.of_old_formula template.quals_bounds) ])
