...
module Make (Cfg : WFPredicate.Config.ConfigType) (Arg : ArgType) :
  ...
  let rec init_quals tag quals =
    ...
    let params_x, params_y =
      (params_of_idx_l idxl, params_of_idx_r idxl idxr)
    in
    let quals, _terms = adjust_quals_terms ~tag (quals, Set.Poly.empty) in
    let qualsl = Templ.quals_over_set quals (params @ params_x) in
    let qualsr = Templ.quals_over_set quals (params @ params_y) in
    let subst_y =
  ...
  let update_hspace ~tag (hspace : HypSpace.hspace) =
    ignore tag;
    let qualsl =
    ...
    in
    let hspace = HypSpace.qualifiers_of ~fenv:Arg.fenv !param.depth hspace in
    { hspace with quals = Set.Poly.union_list [ hspace.quals; qualsl; qualsr ] }

  let gen_template ~tag ~ucore:_ (hspace : HypSpace.hspace) =
    let idxl, idxr = Option.value_exn tag in
