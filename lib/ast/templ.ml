...
let gen_simplified_wf_predicate tfp tsfp params : templ_wf_pred =
  ...
  let sel_terms_x =
    ...
    @@ List.filter ~f:T_irb.is_sirb
    @@ size_fun_apps params (*ToDo*) @ tfp.terms
    @ List.concat_map quals ~f:(function
        | Atom (Atom.App (Predicate.Psym psym, [ lhs; rhs ], _), _) -> (
  ...
  let size = List.length irb_params / 2 in
  let txs, tys =
    let consts = List.unique @@ List.filter tfp.consts ~f:T_irb.is_sirb in
    let irb_params_x, irb_params_y = List.split_n irb_params size in
    ( (if true then [] else consts) @ Term.of_sort_env irb_params_x,
      (if true then [] else consts) @ Term.of_sort_env irb_params_y )
  in
